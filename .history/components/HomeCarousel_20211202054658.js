...
const Banner = ({ carouselProducts, selectedItem }) => {
  ...
  useEffect(() => {
//write an equation such that if the window is 329px wide, the slide percentage is 68.7% and that if the window is 368px wide, the slide percentage is 61.415%

const log = Math.log(windowSize.width / 329) / Math.log(1.7);
const slidePercentage2 = Math.round((log * 68.7) / log);
setSlidePercentage(slidePercentage2);
return slidePercentage
// const slidePercentage = (windowSize.width / 329) * 68.7;
