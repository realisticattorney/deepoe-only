...
const Banner = ({ product , selectedVariant}) => {
  ...
  const imageColorSrc = product.images.edges.filter(node => node.node.altText === selectedVariant.options.Color || node.node.altText === 'All').map(node => node.node.originalSrc)
